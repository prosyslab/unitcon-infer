...
let write_field path field new_val location ~ref addr astate =
  let* astate, field_addr =
    PulseOperations.eval_access path Write location addr (FieldAccess field) astate
  in
  let astate =
    add_dependency_for_write_field (fst field_addr)
      ~ref:(Option.map ~f:(fun x -> fst x) ref)
      (fst addr) (fst new_val) astate
  in
...
module Object = struct
  ...
  let clone src_pointer_hist : model =
   fun {path; location; ret= ret_id, _} astate ->
    ...
    in
    let astate = add_dependency_for_load_field (fst obj) (fst src_pointer_hist) astate in
    let<+> astate, obj_copy = PulseOperations.shallow_copy path location obj astate in
    let astate =
      PulseOperations.write_id ret_id
        (fst obj_copy, Hist.add_event path event (snd obj_copy))
        astate
    in
...
module Iterator = struct
  let constructor ~desc init : model =
   fun {path; location; ret} astate ->
    ...
    in
    let astate = add_dependency_for_load_field (fst ref) (fst init) astate in
    PulseOperations.write_id (fst ret) ref astate
    |> add_dependency_for_return (Var.of_id (fst ret)) (fst ref)
  ...
  let next ~desc iter : model =
   fun {path; location; ret} astate ->
    ...
    in
    let astate =
      add_dependency_for_load_field curr_elem_val (fst iter) astate
      |> add_dependency_for_load_field curr_elem_val curr_index
    in
    let<+> astate =
    ...
    PulseOperations.write_id (fst ret)
      ...
      astate
    |> add_dependency_for_return (Var.of_id (fst ret)) curr_elem_val
  ...
  let remove ~desc iter : model =
   fun {path; location} astate ->
    ...
    let astate =
      add_dependency_for_load_field (fst arr) (fst iter) astate
      |> add_dependency_for_load_field (fst arr) curr_index
    in
    let<+> astate =
    ...
    let delegated_state =
      let<+> post =
        Option.value_map delegation_opt ~default:(Ok post) ~f:(fun obj ->
            write_field path PulseOperations.ModeledField.delegated_release obj location ~ref:None
              this_obj post )
      in
      post
    in
...
module Collection = struct
   ...
   fun {path; location} astate ->
    ...
    let<*> astate =
      write_field path fst_field
        ...
        location ~ref:None fresh_val astate
    in
    let<*> astate =
  ...
  let add_common ~desc coll new_elem ?new_val : model =
   fun {path; location; ret= ret_id, _} astate ->
    ...
    let<*> astate =
      ...
      >>| add_dependency_for_return (Var.of_id ret_id) ret_value
    in
    ...
    if PulseArithmetic.is_known_zero astate is_empty_val then astate |> Basic.ok_continue
      ...
      let<*> astate =
        write_field path is_empty_field
          ...
          location ~ref:(Some coll) coll_val astate
  ...
  let update path coll new_val new_val_hist event location ret_id astate =
    ...
    let astate1 =
      write_field path fst_field
        ...
        location ~ref:None coll astate'
      >>| PulseOperations.write_id ret_id fst_val
      >>| add_dependency_for_return (Var.of_id ret_id) (fst fst_val)
      |> Basic.map_continue
    in
    ...
    let astate2 =
      ...
      >>| add_dependency_for_return (Var.of_id ret_id) (fst snd_val)
      |> Basic.map_continue
    in
  ...
  let remove_elem_found path coll coll_val elem field_addr field_val ret_id location event astate =
    ...
    |> add_dependency_for_return (Var.of_id ret_id) ret_val
  ...
  let remove_obj path ~desc coll (elem, _) location ret_id astate =
    ...
    let astate1 =
      remove_elem_found path coll coll_val elem fst_addr fst_val ret_id location event astate
      |> Basic.map_continue
    in
    (* case2: given element is equal to snd_field *)
    let astate2 =
      remove_elem_found path coll coll_val elem snd_addr snd_val ret_id location event astate
      |> Basic.map_continue
    in
  ...
  let clear ~desc coll : model =
   fun {path; location} astate ->
    ...
    in
    let<*> astate =
      write_field path fst_field (null_val, hist) location ~ref:(Some coll) coll_val astate
    in
    let<*> astate =
      write_field path snd_field (null_val, hist) location ~ref:(Some coll) coll_val astate
    in
    let<*> astate =
      write_field path is_empty_field (is_empty_val, hist) location ~ref:(Some coll) coll_val astate
    in
    let<+> astate =
      PulseArithmetic.and_eq_int null_val IntLit.zero astate
...
module Map = struct
  ...
  let key_field = mk_java_field pkg_name class_name "__infer_model_backing_map_key"

  let value_field = mk_java_field pkg_name class_name "__infer_model_backing_map_value"

  let is_empty_field = mk_java_field pkg_name class_name "__infer_model_backing_map_empty"
  ...
  let init ~desc this : model =
   fun {path; location} astate ->
    ...
    (* The two internal fields are initially set to null *)
    let<*> astate =
      write_field path key_field
        (init_value, Hist.single_event path event)
        location ~ref:None fresh_val astate
    in
    let<*> astate =
      write_field path value_field
        (init_value, Hist.single_event path event)
        location ~ref:None fresh_val astate
    in
    (* The empty field is initially set to true *)
  ...
  let put ~desc coll key value : model =
   fun {path; location; ret= ret_id, _} astate ->
    let event = Hist.call_event path location desc in
    let<*> astate, coll_val =
      PulseOperations.eval_access path Read location coll Dereference astate
    in
    (* writes key to key_field *)
    let<*> astate = write_field path key_field key location ~ref:(Some coll) coll_val astate in
    (* writes value to value_field *)
    let<*> astate = write_field path value_field value location ~ref:(Some coll) coll_val astate in
    (* Map.put returns a value, in this case the return always returns value *)
    PulseOperations.write_id ret_id (fst value, Hist.single_event path event) astate
    |> add_dependency_for_return (Var.of_id ret_id) (fst value)
    |> Basic.ok_continue
  ...
  (* Auxiliary function that updates the state by:
     (1) assuming that value to be removed is equal to field value
     (2) assigning field to null value
     Map.remove should return a value. In this case, the return val is value_val *)
  let remove_found path coll coll_val key key_addr key_val value_addr value_val ret_id location
      event astate =
    let null_val = AbstractValue.mk_fresh () in
    let is_empty_val = AbstractValue.mk_fresh () in
    let* astate =
      write_field path is_empty_field
        ...
        location ~ref:(Some coll) coll_val astate
    in
    let* astate = PulseArithmetic.and_eq_int null_val IntLit.zero astate in
    let* astate =
      PulseArithmetic.prune_binop ~negated:false Binop.Eq (AbstractValueOperand key)
        (AbstractValueOperand key_val) astate
    in
    let* astate =
      PulseOperations.write_deref path location ~ref:key_addr
        ~obj:(null_val, Hist.single_event path event)
        astate
    in
    let+ astate =
      PulseOperations.write_deref path location ~ref:value_addr
        ~obj:(null_val, Hist.single_event path event)
        astate
    in
    PulseOperations.write_id ret_id (value_val, Hist.single_event path event) astate
    |> add_dependency_for_return (Var.of_id ret_id) value_val


  let remove_obj path ~desc coll (key, _) location ret_id astate =
    let event = Hist.call_event path location desc in
    let<*> astate, coll_val =
      PulseOperations.eval_access path Read location coll Dereference astate
    in
    let<*> astate, key_addr, (key_val, _) = load_field path key_field location coll_val astate in
    let<*> astate, value_addr, (value_val, _) =
      load_field path value_field location coll_val astate
    in
    (* case1: given key is equal to key_field *)
    let astate1 =
      remove_found path coll coll_val key key_addr key_val value_addr value_val ret_id location
        event astate
      |> Basic.map_continue
    in
    (* case 2: given key is not equal to the key_field *)
    let astate2 =
      PulseArithmetic.prune_binop ~negated:true Binop.Eq (AbstractValueOperand key)
        (AbstractValueOperand key_val) astate
      |> Basic.map_continue
    in
    [astate1; astate2]
  ...
  let remove ~desc args : model =
   fun {path; location; ret= ret_id, _} astate ->
    match args with
    | [ {ProcnameDispatcher.Call.FuncArg.arg_payload= coll_arg}
      ; {ProcnameDispatcher.Call.FuncArg.arg_payload= key} ] ->
        remove_obj path ~desc coll_arg key location ret_id astate
    | _ ->
        astate |> Basic.ok_continue
  ...
  let clear ~desc coll : model =
   fun {path; location} astate ->
    ...
    in
    let<*> astate =
      write_field path key_field (null_val, hist) location ~ref:(Some coll) coll_val astate
    in
    let<*> astate =
      write_field path value_field (null_val, hist) location ~ref:(Some coll) coll_val astate
    in
    let<*> astate =
      write_field path is_empty_field (is_empty_val, hist) location ~ref:(Some coll) coll_val astate
    in
  ...
  let get_key_coll_not_known_empty key key_val value_val ret_id path event astate =
    ...
    let null_val = AbstractValue.mk_fresh () in
    let astate1 =
      PulseArithmetic.prune_binop ~negated:true Eq (AbstractValueOperand key)
        (AbstractValueOperand key_val) astate
      ...
      |> Basic.map_continue
    in
    (* case 2: given key is equal to key_field *)
    let astate2 =
  ...
  let get ~desc coll (key, _) : model =
   fun {path; location; ret= ret_id, _} astate ->
    ...
    let astate1 =
      get_key_coll_is_empty path is_empty_val true_val event location ret_id astate
      |> Basic.map_continue
    in
    (* case 2: map is not known to be empty *)
    let astates2 =
      let<*> astate2, _, (key_val, _) = load_field path key_field location coll_val astate in
      let<*> astate2, _, (value_val, _) = load_field path value_field location coll_val astate2 in
      let<*> astate2 =
...
module Integer = struct
  ...
  let equals this arg : model =
   fun {path; location; ret= ret_id, _} astate ->
    ...
    |> add_dependency_for_return (Var.of_id ret_id) binop_addr


  let int_val this : model =
   fun {path; location; ret= ret_id, _} astate ->
    let<*> astate, _int_addr1, (int_value, hist) = load_backing_int path location this astate in
    PulseOperations.write_id ret_id (int_value, Hist.hist path hist) astate
    |> add_dependency_for_return (Var.of_id ret_id) int_value
    |> Basic.ok_continue
  ...
  let value_of init_value : model =
   fun {path; location; ret= ret_id, _} astate ->
    ...
    |> add_dependency_for_return (Var.of_id ret_id) (fst new_alloc)
end
...
module Preconditions = struct
  let check_not_null (address, hist) : model =
   fun {location; path; ret= ret_id, _} astate ->
    ...
    |> add_dependency_for_return (Var.of_id ret_id) address
...
module AtomicBoolean = struct
  ...
  let update path bool new_val _new_val_hist event location _ret_id astate =
    ...
    let<*> astate' =
      write_field path internal_bool
        ...
        ~ref:(Some bool) location bool_val astate
    in
  ...
  let get this : model =
   fun {path; location; ret= ret_id, _} astate ->
    ...
    PulseOperations.write_id ret_id (bool_value, Hist.hist path hist) astate
    |> add_dependency_for_return (Var.of_id ret_id) bool_value
    |> Basic.ok_continue
end
...
let matchers : matcher list =
  ...
  ; +map_context_tenv PatternMatch.Java.implements_map
    &:: "<init>" <>$ capt_arg_payload $--> Map.init ~desc:"Map.init()"
  ; +map_context_tenv PatternMatch.Java.implements_map
    &:: "put" <>$ capt_arg_payload $+ capt_arg_payload $+ capt_arg_payload
    $--> Map.put ~desc:"Map.put()"
  ; +map_context_tenv PatternMatch.Java.implements_map
    &:: "remove" &++> Map.remove ~desc:"Map.remove()"
  ; +map_context_tenv PatternMatch.Java.implements_map
    &:: "get" <>$ capt_arg_payload $+ capt_arg_payload $--> Map.get ~desc:"Map.get()"
  ; +map_context_tenv PatternMatch.Java.implements_map
    &:: "containsKey" <>$ capt_arg_payload $+ capt_arg_payload
    $--> Map.get ~desc:"Map.containsKey()"
  ; +map_context_tenv PatternMatch.Java.implements_map
    &:: "isEmpty" <>$ capt_arg_payload
    $--> Map.is_empty ~desc:"Map.isEmpty()"
  ; +map_context_tenv PatternMatch.Java.implements_map
    &:: "clear" <>$ capt_arg_payload $--> Map.clear ~desc:"Map.clear()"
